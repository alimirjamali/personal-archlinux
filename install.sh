...
pkgs="$pkgs grub grub-customizer"		# Obviously we need a boot loader (and we like to customize it).
# pkgs="$pkgs grub-btrfs" 			# btrfs snapshots for grub
pkgs="$pkgs os-prober"				# Detecting other OSes for grub
pkgs="$pkgs syslinux" 				# syslinux to chainload HDT (Hardware Detection Tool)
...
pkgs="$pkgs iftop nethogs nload"		# Network usage monitoring tools
